...
erDiagram
    ...
    CONCEPT ||--o{ CONDITION_OCCURRENCE : defines
    CONCEPT ||--o{ DRUG_EXPOSURE : defines
    CONCEPT ||--o{ PROCEDURE_OCCURRENCE : defines
    CONCEPT ||--o{ MEASUREMENT : defines
    CONCEPT ||--o{ PERSON : defines
...
This shows how a patient's data connects across CDM tables:

```{mermaid}
flowchart TD
    subgraph Patient Record
        P[person<br/>Demographics]
    end

    subgraph Time Boundaries
        OP[observation_period<br/>When we have data]
    end

    subgraph Clinical Events
        V[visit_occurrence<br/>Encounters]
        CO[condition_occurrence<br/>Diagnoses]
        DE[drug_exposure<br/>Medications]
        PR[procedure_occurrence<br/>Procedures]
        ME[measurement<br/>Labs & Vitals]
    end

    subgraph Vocabulary
        C[concept<br/>Standardized meanings]
    end
    ...
    CO -.-> |condition_concept_id| C
    DE -.-> |drug_concept_id| C
    PR -.-> |procedure_concept_id| C
    ME -.-> |measurement_concept_id| C
    P -.-> |gender_concept_id| C
