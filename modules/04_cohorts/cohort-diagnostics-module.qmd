...
```{r}
#| eval: false
# Concept ID 40481087 = Viral sinusitis (common in Eunomia)
...
FROM (
  ...
  WHERE co.condition_concept_id = 40481087
...
```{r}
#| eval: false
cohortDefinitionSetManual <- data.frame(
  cohortId = c(1, 2),
...
## Export cohorts from Atlas 

Now use WebApi to connect to an ATLAS instance (Demo Atlas in this case) and export the cohort definition set (including the json files).


### Connect to ATLAS WebAPI
...
### Export cohort definitions straight to a Cohort DefinitionSet object
...
cohortDefinitionSet <- ROhdsiWebApi::exportCohortDefinitionSet(
  baseUrl       = baseUrl,
  cohortIds     = c(
    1796298, # Viral Sinusitis 
    1796299  # Acute Bronchitis
  ),
  generateStats = TRUE
)
